...
function SourceVisualCarousel({ source, visuals }: { source: Source; visuals: string[] }) {
  const orderedVisuals = useMemo(() => [...new Set([source.imageUrl, ...visuals].filter((value): value is string => Boolean(value)))].slice(0, 6), [source.imageUrl, visuals]);
...
function SourceDossier({ source, visuals, isDarkMode, onClose, onDisintegrate }: { source: Source; visuals: string[]; isDarkMode: boolean; onClose: () => void; onDisintegrate: (source: Source) => void }) {
  ...
  const visualStatus = hasThumbnail ? 'Source visual metadata available' : source.imageUrl ? 'Source visual unavailable in this browser' : source.contentInspected ? 'No source visual metadata exposed' : 'Source visual metadata not inspected';
  return <motion.aside initial={{ x: -18, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -18, opacity: 0 }} className={cn('source-dossier', isDarkMode ? 'dossier-dark' : 'dossier-light')}>
    ...
    <SourceVisualCarousel source={source} visuals={visuals}/>
    ...
    <details className="source-trace-details"><summary>Source trace & observed signals <ChevronDown size={13}/></summary><dl><div><dt>Original link</dt><dd><a href={source.url} target="_blank" rel="noreferrer">{sourceHost}<ArrowRight size={11}/></a></dd></div><div><dt>Returned citations</dt><dd>{source.citations ?? 'Not indexed'}</dd></div><div><dt>Observed active links</dt><dd>{typeof source.observedReferenceCount === 'number' ? `${source.observedReferenceCount} linked traces` : 'Not fetched yet'}</dd></div><div><dt>Directness</dt><dd>{typeof profile?.directness === 'number' ? `${profile.directness}/100` : 'Being assessed'}</dd></div><div><dt>Provider</dt><dd>{source.provider === 'gemini_google' ? 'Google-grounded lead' : 'OpenAI web discovery'}</dd></div></dl></details>
...
function ResultsToolbar({ isDarkMode, viewMode, labelMode, summarising, expanding, canExpand, onViewMode, onLabelMode, onSummary, onExpand, onSave, onExport, onLibrary, onInfo, onTheme, onNewSearch }: { isDarkMode: boolean; viewMode: '3d' | '2d'; labelMode: 'hover' | 'all'; summarising: boolean; expanding: boolean; canExpand: boolean; onViewMode: (mode: '3d' | '2d') => void; onLabelMode: () => void; onSummary: () => void; onExpand: () => void; onSave: () => void; onExport: () => void; onLibrary: () => void; onInfo: () => void; onTheme: () => void; onNewSearch: () => void }) {
...
export default function App() {
  ...
  const activeVisuals = useMemo(() => [...new Set((result?.branches.flatMap((branch) => branch.sources.map((source) => source.imageUrl).filter((image): image is string => Boolean(image))) || []))], [result]);
