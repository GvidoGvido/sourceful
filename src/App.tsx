...
function ResearchBriefPanel({ summary, isDarkMode, onClose }: { summary: string; isDarkMode: boolean; onClose: () => void }) {
  return <motion.aside initial={{ y: 18, opacity: 0, filter: 'blur(7px)' }} animate={{ y: 0, opacity: 1, filter: 'blur(0px)' }} exit={{ y: 18, opacity: 0, filter: 'blur(7px)' }} className={cn('research-brief-panel', isDarkMode ? 'dossier-dark' : 'dossier-light')}><div className="dossier-topline"><span><Sparkles size={14}/> Research briefing</span><button onClick={onClose} title="Close briefing"><X size={17}/></button></div><p>{summary}</p></motion.aside>;
}
...
function ResultsToolbar({ isDarkMode, viewMode, labelMode, summarising, onViewMode, onLabelMode, onSummary, onSave, onExport, onLibrary, onInfo, onTheme, onNewSearch }: { isDarkMode: boolean; viewMode: '3d' | '2d'; labelMode: 'hover' | 'all'; summarising: boolean; onViewMode: (mode: '3d' | '2d') => void; onLabelMode: () => void; onSummary: () => void; onSave: () => void; onExport: () => void; onLibrary: () => void; onInfo: () => void; onTheme: () => void; onNewSearch: () => void }) {
  return <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="result-toolbar"><div className={`result-brand text-xl font-serif tracking-[0.22em] uppercase ${isDarkMode ? 'text-amber-50' : 'text-slate-900'}`}>Sourceful</div><div className="result-actions"><div className={`result-mode-cluster flex items-center rounded-full border p-1 backdrop-blur-md ${isDarkMode ? 'bg-slate-900/75 border-white/10' : 'bg-white/80 border-slate-200'}`}><button onClick={() => onViewMode('3d')} className={`view-mode-button ${viewMode === '3d' ? 'selected' : ''}`}><Box size={13}/><span>Discovery</span></button><button onClick={() => onViewMode('2d')} className={`view-mode-button ${viewMode === '2d' ? 'selected' : ''}`}><PanelsTopLeft size={13}/><span>Board</span></button>{viewMode === '3d' && <button onClick={onLabelMode} className={`view-mode-button ${labelMode === 'all' ? 'selected' : ''}`} title={labelMode === 'all' ? 'Labels are visible for every node' : 'Labels appear only on hover'}><Tags size={13}/><span>Labels: {labelMode === 'all' ? 'All' : 'Hover'}</span></button>}<button onClick={onSummary} className="view-mode-button" title="Generate a careful research briefing"><Sparkles size={13}/><span>{summarising ? 'Writing' : 'Brief'}</span></button><button onClick={onSave} className="view-mode-button" title="Save this knowledge graph to your browser"><Save size={13}/><span>Save</span></button><button onClick={onExport} className="view-mode-button" title="Export claims and sources as CSV"><Download size={13}/><span>CSV</span></button></div><div className="result-utility"><button onClick={onLibrary} title="Saved research library"><FolderOpen size={16}/></button><button onClick={onInfo} title="How Sourceful evaluates evidence"><Info size={16}/></button><button onClick={onTheme} title="Toggle theme">{isDarkMode ? <Sun size={16}/> : <Moon size={16}/>}</button></div><button onClick={onNewSearch} className={`result-new-search text-sm transition-colors px-4 py-2 rounded-full backdrop-blur-md border ${isDarkMode ? 'text-white/70 hover:text-white bg-slate-900/70 hover:bg-slate-900 border-white/10' : 'text-slate-600 hover:text-slate-900 bg-white/80 hover:bg-white border-slate-200'}`}><span>New Search</span><Search size={15}/></button></div></motion.div>;
}
...
export default function App() {
  ...
  const restoreArtifact = (artifact: SavedArtifact) => { setResult(artifact.result); setQuery(artifact.query); setModel(artifact.model); setSummary(artifact.summary || ''); setAppState('results'); setLibraryOpen(false); };
  const selectSource = (source: Source) => { setSelectedClaim(null); setSelectedSource(source); };
  const selectClaim = (claim: Branch) => { setSelectedSource(null); setSelectedClaim(claim); };
  ...
  const resetSearch = () => { setAppState('idle'); setResult(null); setQuery(''); setSelectedFile(null); setSelectedSource(null); setSelectedClaim(null); setSummary(''); setConfigOpen(false); setGuideVisible(true); setSearchPanelKey((value) => value + 1); };
    ...
    <div className="min-h-screen sourceful-shell bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white font-sans overflow-hidden relative selection:bg-amber-500/30 transition-colors duration-500">
      ...
      {createPortal(<div className="sourceful-viewport-ui">
        {appState !== 'results' && <div className="utility-controls flex items-center gap-4">
          <button onClick={() => setLibraryOpen(true)} className={`p-2 transition-colors ${isDarkMode ? 'text-white/50 hover:text-white' : 'text-slate-500 hover:text-slate-800'}`} title="Saved research library"><FolderOpen size={20} /></button>
          <button onClick={() => setInfoOpen(true)} className={`p-2 transition-colors ${isDarkMode ? 'text-white/50 hover:text-white' : 'text-slate-500 hover:text-slate-800'}`} title="How Sourceful evaluates evidence"><Info size={20} /></button>
