...
export function DiscoveryUniverse({ data, isDarkMode, labelMode, onSourceSelect, onClaimSelect, disintegratingSourceUrl, disintegratingClaim, onDisintegrationComplete }: Props) {
  ...
  return <div className="discovery-universe"><Canvas onPointerMissed={() => setActiveNode(null)} dpr={[1, 2]} camera={{ position: [0, 0, 10], fov: 45 }} gl={{ antialias:true, alpha:true }} style={{ touchAction: 'none' }}><color attach="background" args={[isDarkMode ? '#090d13' : '#f6f4ef']}/><fog attach="fog" args={[isDarkMode ? '#090d13' : '#f6f4ef', 8, 19]}/><ambientLight intensity={.34}/><pointLight position={[0, 1, 4]} intensity={42} color="#d9ad50"/><pointLight position={[-4, -3, 3]} intensity={19} color="#5ca4d5"/><pointLight position={[4, 2, -3]} intensity={12} color="#77c5a0"/><Sparkles count={280} scale={[18,11,9]} size={1.65} speed={.25} color={isDarkMode ? '#ead083' : '#a67b24'}/>
    ...
    <OrbitControls enablePan minDistance={6.5} maxDistance={15} autoRotate autoRotateSpeed={.22} enableDamping dampingFactor={.06}/>
