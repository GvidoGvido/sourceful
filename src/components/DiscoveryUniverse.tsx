...
function Orb({ nodeId, active, selected = false, lineage = false, labelMode, onFocus, position, color, size, label, detail, preview, citedText, imageUrl, onClick, order = 0, disintegrating = false, onDisintegrationComplete }: { nodeId: string; active: boolean; selected?: boolean; lineage?: boolean; labelMode: 'hover' | 'all'; onFocus: (id: string | null, releasedNodeId?: string) => void; position: [number,number,number]; color: string; size: number; label: string; detail: string; preview?: string; citedText?: string; imageUrl?: string; onClick?: () => void; order?: number; disintegrating?: boolean; onDisintegrationComplete?: () => void }) {
  ...
  // Moving from the orb to its nearby HTML plaque briefly crosses rendering layers.
  // A short grace period prevents flicker without leaving a preview open after exit.
  const scheduleBlur = () => { cancelBlur(); blurTimer.current = window.setTimeout(() => { setPreviewHovered(false); onFocus(null, nodeId); blurTimer.current = null; }, 260); };
  // A selected dossier and a hover preview have separate lifetimes: clicking keeps
  // the dossier open, but only the node currently under the pointer owns a plaque.
  ...
  // Html's `center` prop already offsets its wrapper by half of the plaque's height.
  // Positioning at the sphere edge therefore puts the visible plaque right above it.
  const previewAnchor = size + Math.max(.045, size * .055);
  const selectNode = () => { cancelBlur(); onFocus(nodeId); onClick?.(); };
  return <group ref={group} position={position}>
    <group ref={visual}><EnergyAura color={color} size={size} hovered={energized} selected={selected}/>{selected && <SelectionHalo size={size}/>}<mesh raycast={() => undefined}><sphereGeometry args={[size, 72, 72]}/><PlasmaSurface color={color} hovered={energized}/></mesh><pointLight color={selected ? '#ffe39a' : energized ? '#ffe39a' : lineage ? '#f8d47c' : color} intensity={selected ? 10.5 : energized ? 4.9 : lineage ? 2.85 : 2.3} distance={size * (selected ? 10 : 7)}/><Html zIndexRange={[160, 0]} distanceFactor={12} center position={[0, previewAnchor, 0]} style={{ pointerEvents: 'none' }}><Preview label={label} detail={detail} preview={preview} citedText={citedText} imageUrl={imageUrl} visible={labelMode === 'all' || selected || previewFocused} focused={previewFocused} onPreviewEnter={() => { cancelBlur(); setPreviewHovered(true); onFocus(nodeId); }} onPreviewLeave={() => { setPreviewHovered(false); scheduleBlur(); }} onPreviewSelect={selectNode}/></Html></group>
    <mesh onPointerOver={(event) => { if (disintegrating) return; event.stopPropagation(); cancelBlur(); setHovered(true); onFocus(nodeId); }} onPointerOut={(event) => { event.stopPropagation(); setHovered(false); scheduleBlur(); }} onClick={(event) => { if (disintegrating) return; event.stopPropagation(); selectNode(); }}><sphereGeometry args={[hitRadius, 24, 24]}/><meshBasicMaterial transparent opacity={0} depthWrite={false}/></mesh>
...
export function DiscoveryUniverse({ data, isDarkMode, labelMode, driftPaused = false, onSourceSelect, onClaimSelect, selectedSourceId, selectedClaimId, disintegratingSourceId, disintegratingClaimId, onDisintegrationComplete }: Props) {
  ...
  return <div className="discovery-universe"><Canvas onPointerMissed={() => setActiveNode(null)} dpr={[1, 2]} camera={{ position: [0, 0, cameraDistance], fov: 45 }} gl={{ antialias:true, alpha:true }} style={{ touchAction: 'none', cursor: activeNode ? 'pointer' : 'default' }}><color attach="background" args={[isDarkMode ? '#090d13' : '#f6f4ef']}/><fog attach="fog" args={[isDarkMode ? '#090d13' : '#f6f4ef', sceneRadius + 4, sceneRadius * 3.25]}/><ambientLight intensity={.34}/><pointLight position={[0, 1, 4]} intensity={42} color="#d9ad50"/><pointLight position={[-4, -3, 3]} intensity={19} color="#5ca4d5"/><pointLight position={[4, 2, -3]} intensity={12} color="#77c5a0"/><Sparkles count={Math.min(540, 280 + totalSources * 7)} scale={[sceneRadius * 3.1,sceneRadius * 1.9,9]} size={1.65} speed={.25} color={isDarkMode ? '#ead083' : '#a67b24'}/>
    <Orb nodeId="core" active={activeNode === 'core'} lineage={selectedBranchIndex >= 0} labelMode={labelMode} onFocus={focusNode} position={[0,0,0]} color="#d4a64b" size={1.08} label={`CORE QUESTION · ASSESSMENT ${data.confidenceScore}%`} detail={data.coreConcept} preview={data.biasAnalysis} order={0}/>
